...
export function assessControls(controls, applicability, evidence, dossier, antiPatterns) {
  ...
  return controls.filter((control) => controlInScope(control, dossier)).map((control) => {
    ...
    if (applicable === "NOT_APPLICABLE") {
      return { id: newId("control-assessment"), controlId: control.id, domain: control.domain, title: control.title, applicability: applicable, state: "NOT_APPLICABLE", targetState: control.targetState, meetsTarget: true, evidenceIds: [], contradictions: [], gap: null };
    }
    const supporting = evidence.filter((item) => item.controlIds.includes(control.id) && item.polarity === "SUPPORT" && !item.stale);
    const risks = evidence.filter((item) => item.polarity === "RISK" && (item.controlIds.includes(control.id) || item.antiPatternIds.some((id) => antiPatterns.find((entry) => entry.id === id)?.relatedControlIds.includes(control.id))));
    ...
    const gap = meetsTarget ? null : {
      id: newId("gap"), domain: control.domain, controlId: control.id, title: `${control.title} is not sufficiently established`,
    ...
    return {
      id: newId("control-assessment"), controlId: control.id, domain: control.domain, title: control.title,
...
export function assessAntiPatterns(antiPatterns, evidence, controlAssessments) {
  return antiPatterns.map((antiPattern) => {
    ...
    return {
      id: newId("antipattern-assessment"), antiPatternId: antiPattern.id, domain: antiPattern.domain,
      title: antiPattern.title, severity: antiPattern.severity, state,
      evidenceIds: state === "CONFIRMED_PRESENT" ? riskEvidence.map((item) => item.id) : absenceEvidence.map((item) => item.id),
