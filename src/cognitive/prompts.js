...
export const PROMPT_VERSIONS = Object.freeze({
  ...
  domain: "domain-assessment-3.1.0",
...
Assess only the listed ASSESSMENT_WORK_ITEMS in this bounded batch. These items are the loaded A–F assessment instrument: capabilities (A1–F5 / CTRL-A1–CTRL-F5), paired anti-patterns (AP-A1–AP-F5), and their three primary questions (DEFINITION_AND_INTENT, IMPLEMENTATION_AND_OPERATION, EVIDENCE_AND_EFFECTIVENESS). Each work item's question or title is the exact wording to assess — do not rewrite, merge, skip or invent questions. Generate candidate claims, not conclusions. Control support and anti-pattern assessment are separate streams. Every listed work item has been searched in the supplied packet scope. If no relevant evidence is found, return no claim for that work item; the Engine will record an explicit assessed-but-unknown result. Never treat missing evidence as support.
DOMAIN_CONTROLS and DOMAIN_ANTIPATTERNS include titles, definitions, indicators and the same primary questions. They are the assessment instrument, not a published Knowledge Base. Do not invent evidence rules, atomic tests, finding-definition IDs, legal conclusions, or approval decisions. When finding-definition IDs are not supplied, leave findingDefinitionIds empty and proposedFindingState null.
Every factual claim needs at least one exact source-unit ID and an exact, short verbatim quote copied from that source unit. For a capability question or control, include the parent assessment-object ID (A1, B2, …) in assessmentObjectIds and the compiled control ID (CTRL-A1, …) in controlIds. For an anti-pattern question or anti-pattern, include the anti-pattern ID (AP-A1, …) in antiPatternIds. When assessing a question, also include that question ID (A1-Q1, AP-A1-Q2, …) in assessmentObjectIds. Map claims to exact finding-definition IDs only when those IDs exist in the supplied objects.
Missing evidence is UNKNOWN. A test source supports TESTED only when it contains successful execution results and adequate scope; test code alone can support at most IMPLEMENTED. Code or configuration can support at most IMPLEMENTED. Human validation requires an attributable human review record and does not equal formal approval. Use ABSENCE_TEST only when the evidence records test scope, method, execution date, system version, successful result and limitations.
Evaluate contextual relevance before creating a claim. Documentation describing a desired control, a knowledge-base rule, a test fixture, an example, or an unrelated domain implementation is not evidence that the assessed solution implements that control. Generic keyword overlap is never sufficient. Use CONTROL_SUPPORT only when the cited artifact performs or records the precise assessed control for this solution. For every returned claim, provide every schema field: use empty arrays for no mappings and null for proposedFindingState or absenceTest when not applicable. Never invent an absence test.
...
Task: independently verify one candidate governance claim. Check whether the cited source locations and exact quotes support the precise statement, whether the claim scope or evidence strength is overstated, whether the control/requirement/anti-pattern/finding mappings are appropriate, and whether any supplied unit conflicts. Do not invent finding-definition IDs, evidence rules or atomic tests that are not already on the candidate claim. Return the strongest accepted assurance state and explicit quote, scope and mapping statuses. Classify as SUPPORTED, PARTIAL, UNSUPPORTED, CONFLICTING, or NOT_VERIFIABLE. Always provide acceptedAssuranceState; use UNKNOWN when no state is justified. Always provide mappingStatus, scopeStatus, and quoteStatus; use NOT_CHECKED only when the supplied evidence prevents that check. Do not decide readiness.
