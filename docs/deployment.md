...
2. Use explicitly requested advisory acquisition test runs to evaluate the configured `WORKHORSE` retrieval-planning and `REASONER` Intake-proposal candidates. These actions transmit only their bounded privacy-safe contracts and retain no Intake or analysis authority. The dedicated benchmark remains available with `BENCHMARK_CONFIRM_LIVE_CALLS=true` for controlled comparison against the qualification floors.
