...
2. Use explicitly requested advisory acquisition test runs to evaluate the configured `WORKHORSE` retrieval-planning and `REASONER` Intake-proposal candidates. Retrieval sends only safe metrics and controlled signals; invalid returned search terms are removed locally. Proposal requests send only deterministic summaries, field-mapped `semantic-intake-evidence-1.0.0` observations and explicitly selected controlled facts. These actions retain no Intake or analysis authority. The dedicated benchmark remains available with `BENCHMARK_CONFIRM_LIVE_CALLS=true` for controlled comparison against the qualification floors.
3. Before decision-ready cognitive analysis, review the six deduplicated `roleSlots` exposed by versioned contract `model-policy-view-1.0.0` at `/api/v2/models`, then add only qualified exact `approvalRef` values to the comma-separated `MODEL_PROFILE_APPROVALS`. Each slot includes its assigned stages and qualification state. Analysis routing always fails closed when a primary or fallback role assignment is absent or its provider/model identity changes; `NODE_ENV` cannot bypass qualification.
...
`/health` returns the active knowledge version, manifest hash, source, entry counts and a non-secret `cognitiveReadiness` summary. Process health remains distinct from cognitive readiness: `CONFIGURATION_REQUIRED` reports credential counts, qualification counts, topology status and stable issue codes without exposing secret values. A missing, unreachable, malformed, incomplete, or hash-invalid production knowledge snapshot prevents startup.
