...
import { runtimeTacticsFromCatalog } from "./playbook.js";
...
export async function compileRuntimeCollections(validation, outputDirectory, options = {}) {
  ...
  if (typeof options.version !== "string" || !options.version.trim()) throw new Error("Compilation requires an explicit version");
  if (typeof options.releaseStatus !== "string" || !options.releaseStatus.trim()) throw new Error("Compilation requires an explicit releaseStatus");
  ...
  const runtimeTactics = runtimeTacticsFromCatalog(tactics, tactics[0]?.__catalog?.release_status);
  ...
  const report = { schemaVersion: "1.0.0", version: options.version.trim(), releaseStatus: options.releaseStatus.trim(), generatedAt: new Date().toISOString(), validation: { status: validation.status, counts: validation.counts }, files };
