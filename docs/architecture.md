...
The acquisition manifest uses `evidence-acquisition-1.1.0` to record each parsed artifact's acquisition lane, raw-content policy, egress policy, analyzer version and derived-unit lineage. Documents follow `DOCUMENT_LOCAL_ANALYSIS` or `DOCUMENT_LOCAL_OCR_ANALYSIS`; `document-evidence-summary-1.0.0` exposes only allow-listed document classes, formats, coarse dimensions, topic/risk signals and limitations—never source text, names, values or quotes. Sparse PDF pages are rasterized and supported images are read locally under the bounded `local-ocr-evidence-1.0.0` contract using pinned English language data: page, engine, language, confidence and text-free bounding-box provenance stay local, complete OCR text re-enters local DLP, and low-confidence output is excluded from deterministic discovery as `REVIEW_REQUIRED`. Images follow `MEDIA_LOCAL_OCR_ANALYSIS`; `media-evidence-summary-1.1.0` discloses only media type, coarse size and OCR completion state, never pixels, visible text or bounding boxes. Non-text visual content remains explicitly unassessed. Code and configuration are scanned locally without execution; `code-evidence-summary-1.0.0` emits only allow-listed artifact/language classes, coarse size and line ranges, controlled capability/risk signals, an opaque source reference and fixed limitations. CSV and XLSX follow `TABULAR_LOCAL_ANALYSIS`; `tabular-evidence-summary-1.0.0` exposes only coarse row/column/sheet ranges and allow-listed structure, semantic and risk signals—never headers, cells, formulas or sheet names. Static signal detection does not establish documentary claims, runtime behavior, data quality or control effectiveness. Deterministic discovery is governed by `intake-search-registry-1.0.0`, which binds each searchable Intake field to explicit labels, heading aliases, evidence types, source priorities and local extraction strategies for canonical declarations, manifests, README files, architecture sections, structured reports and ownership/RACI tables. Priorities control search order only: every distinct candidate remains conflicting until user resolution. The local-only `intake-candidate-package-1.0.0` records each field's DLP-screened candidate, opaque source references, extraction method, extraction confidence, conflicts, limitations and field-level disclosure decision. Extraction confidence never establishes truth or approval. Sanitized unrestricted free text remains local, is removed from durable checkpoints and cannot become provider eligible. `acquired-fact-package-1.1.0` is hash-linked to and derived from that validated contract: only validated Boolean or allow-listed enum values may be eligible, while free text, dates, unknowns, conflicts and policy-excluded values are represented with `value: null`. Any blocking screening result withholds every acquired value. A user selection creates `acquired-fact-selection-1.0.0`, tied to the package hash and appended only to the optional proposal packet; it never mutates deterministic Intake. The separately user-approved canonical Intake may enter analysis as declared information.
