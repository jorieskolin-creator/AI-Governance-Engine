...
- Evidence acquisition uses versioned lanes. Raw code, configuration and tabular values are inspected locally and replaced in provider-eligible packets by validated summaries containing only controlled signal enums, coarse dimensions, lineage references and explicit limitations.
...
```text
raw source (local only for code/configuration/tabular values)
