...
test("engine identity cannot manufacture human validation or formal approval", async () => {
  const result = await assessSolution(request({
    ...
    sources: [{
      ...
      metadata: { humanActorId: "ENGINE", authority: "LEGAL", controlIds: ["CTRL-A1"] }
    }]
  }));
  assert.equal(control(result, "CTRL-A1").state, "DECLARED");
  assert.equal(result.recommendation.formalApproval, false);
});
...
test("a named authorized human record can establish human validation", async () => {
  const result = await assessSolution(request({
    ...
    sources: [{
      ...
      metadata: { humanActorId: "reviewer-42", authority: "GOVERNANCE", controlIds: ["CTRL-A1"] }
    }]
  }));
  assert.equal(control(result, "CTRL-A1").state, "HUMAN_VALIDATED");
});
...
test("an uploaded approval artifact cannot create FORMALLY_APPROVED state", async () => {
  const result = await assessSolution(request({
    ...
    sources: [{
      ...
      metadata: { humanActorId: "legal-reviewer-7", authority: "LEGAL", controlIds: ["CTRL-A1"] }
    }]
  }));
  assert.equal(control(result, "CTRL-A1").state, "HUMAN_VALIDATED");
...
test("code is implementation evidence, never proof of testing or operation", async () => {
  const result = await assessSolution(request({
    sources: [{
      ...
      metadata: { controlIds: ["CTRL-D1"] }
    }]
  }));
  assert.equal(control(result, "CTRL-D1").state, "IMPLEMENTED");
  assert.equal(control(result, "CTRL-D1").meetsTarget, false);
});

test("a test file is not TESTED evidence without a passed result and scope", async () => {
  const result = await assessSolution(request({
    sources: [{ path: "test/security.test.js", kind: "TEST", content: "assertPromptInjectionBlocked();", metadata: { controlIds: ["CTRL-D3"] } }]
  }));
  assert.equal(control(result, "CTRL-D3").state, "IMPLEMENTED");
});

test("passed test results with explicit scope can establish TESTED", async () => {
  const result = await assessSolution(request({
    sources: [{ path: "results/security-evaluation.json", kind: "TEST", content: "All 120 prompt-injection cases passed.", metadata: { controlIds: ["CTRL-D3"], executionStatus: "PASSED", scope: "120 approved prompt-injection and leakage cases against build abc123" } }]
  }));
  assert.equal(control(result, "CTRL-D3").state, "TESTED");
});
...
test("expired evidence cannot satisfy a control", async () => {
  const result = await assessSolution(request({
    sources: [{
      ...
      metadata: { humanActorId: "security-1", authority: "SECURITY", controlIds: ["CTRL-D3"], validUntil: "2020-01-01T00:00:00.000Z" }
    }]
  }));
  assert.equal(control(result, "CTRL-D3").state, "UNKNOWN");
...
test("contradictory evidence prevents a positive control conclusion", async () => {
  const result = await assessSolution(request({
    sources: [
      { path: "security/review.md", kind: "HUMAN_REVIEW", content: "Threat model reviewed.", metadata: { humanActorId: "security-1", authority: "SECURITY", controlIds: ["CTRL-D3"] } },
      { path: "security/scan.json", kind: "SCAN_RESULT", content: "Threat boundary bypass confirmed.", metadata: { controlIds: ["CTRL-D3"], polarity: "RISK" } }
    ]
  }));
  assert.equal(control(result, "CTRL-D3").meetsTarget, false);
  assert.ok(control(result, "CTRL-D3").contradictions.length > 0);
});
...
test("unknown lifecycle stays unknown, keeps later-stage controls in scope, and treats production access as an experiment-boundary risk", async () => {
  ...
  assert.ok(control(result, "CTRL-D2"));
  assert.ok(control(result, "CTRL-F5"));
