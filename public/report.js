...
function gatesMarkup(summary) {
  ...
  const content = rows.length ? `<div class="report-table-scroll"><table class="report-table"><thead><tr><th>Gate</th><th>State and basis</th><th>Source references</th><th>Clearance requirement</th><th>Authority</th></tr></thead><tbody>${rows.map((gate) => `<tr><td><strong>${escapeHtml(gate.title)}</strong><small class="report-trace">${escapeHtml(gate.code ?? gate.id)}</small></td><td>${status(gate.state)} ${status(gate.basisStatus ?? "DETERMINISTIC_RULE")}</td><td>${escapeHtml(array(gate.availableEvidence).slice(0, 3).map((item) => item.path).join("; ") || (gate.basisStatus === "DECLARED" ? "Submitted declaration" : "No verified evidence recorded"))}</td><td>${array(gate.clearanceCriteria).length ? `<ul class="compact-list">${array(gate.clearanceCriteria).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : "No additional clearance criterion"}</td><td>${escapeHtml(array(gate.requiredHumanAuthorities).map(label).join(", ") || "Deterministic rule")}</td></tr>`).join("")}</tbody></table></div>` : `<p class="report-empty">No deterministic hard gate was triggered.</p>`;
...
function auditMarkup(pkg, summary) {
  const values = [
    ...
    ["Report generator", REPORT_VERSION], ["Source-ingestion manifest", pkg.sourceIngestion?.manifestHash],
    ["Complete evidence", `${summary.auditReference?.evidenceCount ?? 0} record(s) in canonical JSON ${summary.auditReference?.canonicalJsonPath ?? "$.evidence"}`]
  ];
...
function reportBodyMarkup(pkg) {
  ...
  const executiveGaps = array(summary.executiveGapGroups).length ? summary.executiveGapGroups : summary.blockingFindings;
  const blockers = section("09", "Confidence and progression conditions", "Blocking Gaps and Unknowns", listItems(array(executiveGaps), "No blocking gap or unknown was recorded for the declared transition."));
